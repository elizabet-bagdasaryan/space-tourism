...
function Technology() {
	...
	const handleTechClick = async (name: string) => {
		try {
			const response = await axios.get(
				`https://space-tourism-backend-production.up.railway.app/technology/${name}`
			);
	...
	useEffect(() => {
		const fetchFirstTech = async () => {
			try {
				const response = await axios.get(
					"https://space-tourism-backend-production.up.railway.app/technology/Launch vehicle"
				);
				setTechInfo(response.data.technology);
			} catch (error) {
	...
	const handleCircleClick = (circleIndex: number, name: string) => {
		handleCircles(circleIndex);
		handleTechClick(name);
	};
	return (
